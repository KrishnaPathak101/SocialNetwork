...
const Post = ({ post,loading, darkMode, onLike, onSave, onDelete }) => {
...
const TwitterFeed = ({ userpost, userId }) => {
  ...
  useEffect(() => {
    ...
    fetchPosts();
  }, [user, userpost, fetchId]);

  if(loading) {
    return  <div className="flex flex-col gap-4 w-[400px] h-[300px]">
  ...
  return (
    <div className={`twitter-feed max-w-xl mx-auto mt-2 ${darkMode ? 'dark' : ''}`}>
      ...
      {posts.map(post => (
        <Post key={post._id} loading={loading} post={post} darkMode={darkMode} onLike={handleLike} onDelete={handleDelete} />
      ))}
    </div>
  );
};
